...
pub fn main() {
    ...
    for i in 0..all_drawn.len() {
        ...
        let bingos: Vec<Vec<Vec<usize>>> = boards.to_vec().into_iter()
            .filter(|b| check_bingo(b, &drawn))
            .collect();

        if bingos.len() == 1 {
            drawn.iter().for_each(|d| print!("{} ", d));
            println!();
            display_board(&bingos[0]);
            println!("Drawn:{}", all_drawn[i]);
            println!("Sum:{}", sum_unmarked(&bingos[0], &drawn));
            println!("Product:{}", all_drawn[i] * sum_unmarked(&bingos[0], &drawn));
            break;
        }
    }
}
